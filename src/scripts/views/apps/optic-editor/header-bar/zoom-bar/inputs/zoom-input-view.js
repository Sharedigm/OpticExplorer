...
export default RangeInputView.extend({
	...
	template: template(`
		...
		<% if (slider) { %>
		<input type="range" class="desktop-app-only hidden-xs" min="<%= min %>" max="<%= max %>" <% if (step != undefined) { %> step="<%= step %>"<% } %>value="<%= range != undefined? range : min %>" />
		<% } %>
	`),
